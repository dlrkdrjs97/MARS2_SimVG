...
def parse_args():
    ...
    parser.add_argument("--output-coords", action="store_true", help="Output bounding box coordinates as JSON")
...
def inference_detector_with_coords(cfg, model, img, text):
    """Inference image with the detector and return coordinates"""
    from simvg.datasets.pipelines import Compose
    from mmcv.parallel import collate
...
def output_coordinates(predictions, img_metas, args):
    """Output bounding box coordinates as JSON"""
    import json
    
    pred_bboxes = predictions.get("pred_bboxes", [])
    
    if len(pred_bboxes) > 0:
        # Get the first (best) bounding box
    ...
    result = {
        "image_path": args.img,
        "question": args.expression,
        "result": result_coords
    }
    
    # Print JSON to stdout for parsing by batch script
    print("COORDINATES_JSON:", json.dumps(result))
...
def main(args):
    ...
    if args.output_coords:
        output_coordinates(predictions, img_metas, args)
    
    # Always show the visualization results (maintain original functionality)
